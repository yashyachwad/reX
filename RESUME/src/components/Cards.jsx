...
import { Clock, Edit, Trash2, Zap, Award, TrendingUp, Check } from "lucide-react";
    ...
    <div className={cardStyles.profileCard}>
      {user ? (
        <>
          ...
          <div>
            <div className={cardStyles.profileName}>{user.name || "User"}</div>
        ...
        <button className={cardStyles.logoutButton} onClick={handleLogin}>
          Login
        </button>
      )}
    </div>
  ...
  return (
    <div
      className={`group h-auto md:h-[300px] lg:h-[320px] flex flex-col bg-white border-2 overflow-hidden
        ...
        ${isSelected ? "border-violet-500 shadow-lg shadow-violet-500/20 bg-violet-50" : "border-gray-200 hover:bg-violet-300"}`}
      onClick={onSelect}
    >
      {thumbnailImg ? (
        ...
        <div className="w-full h-[200px] flex items-center flex-col juc; bg-gradient-to-br from-violet-50 via-violet-600 to-fuchsia-50 ">
          <div className="w-12 h-12 bg-gradient-to-br from-violet-500 to-fuchsia-500 rounded-2xl fex items-center justify-center mb-3">
              <Edit className="text-wrap" size={20} />
          </div>
