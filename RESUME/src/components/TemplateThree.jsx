...
import { ExternalLink, Github } from "lucide-react"; // ✅ Correct imports
    ...
    >
      ...
      <header className="px-8 pt-8 pb-4 mb-2">
        <div className="text-center">
          ...
          <h2 className="text-lg font-semibold text-gray-700 mb-4">{profileInfo.designation}</h2>
        </div>
        <p className="text-sm text-gray-700 leading-tight mb-4">{profileInfo.summary}</p>
      </header>
            ...
            <ul className="text-xs text-gray-700 space-y-2 pb-2">
              ...
              {contactInfo.linkedin && (
                <li className="flex items-start">
                  ...
                  <a href={contactInfo.linkedin} className="text-blue-600 hover:underline truncate pb-1" title={contactInfo.linkedin}>
              ...
              {contactInfo.github && (
                <li className="flex items-start">
                  ...
                  <a href={contactInfo.github} className="text-blue-600 hover:underline pb-2 truncate" title={contactInfo.github}>
              ...
              {contactInfo.website && (
                <li className="flex items-start">
                  ...
                  <a href={contactInfo.website} className="text-blue-600 hover:underline pb-2 truncate" title={contactInfo.website}>
            ...
            {Object.entries(groupedSkills).map(([category, skillsList]) =>
              skillsList.length > 0 && (
                <div key={category} className="mb-2">
                  {category !== "Other Skills" && <h3 className="text-xs font-semibold italic mb-1">{category}:</h3>}
                  <ul className="text-xs text-gray-700">{skillsList.map((skill, idx) => <li key={idx} className="mb-1">{skill}</li>)}</ul>
                </div>
              )
            )}
          </section>
          ...
          {certifications.length > 0 && (
            <section>
              ...
              <ul className="text-xs text-gray-700 space-y-1">
                {certifications.map((cert, idx) => <li key={idx}>{cert.title} ({cert.year})</li>)}
              </ul>
            </section>
          )}
                ...
                {projects.map((proj, idx) => (
                  <div key={idx} className="text-xs">
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {proj.github && (
                        <a href={proj.github} className="text-blue-600 hover:underline flex items-center text-xs">
                          <Github size={12} /> GitHub
                        </a>
                      )}
                      {proj.liveDemo && (
                        <a href={proj.liveDemo} className="text-blue-600 hover:underline flex items-center text-xs">
